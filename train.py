...
def train_and_evaluate(config: default.Config, workdir: str):
  ...
  vocab_size = (
    encoder.vocab_size()
    if callable(encoder.vocab_size)
    else encoder.vocab_size
  )
  vocab_size = int(vocab_size)
  eos_id = (
    encoder.eos_id if hasattr(encoder, 'eos_id') else temperature_sampler.EOS_ID
  )
  ...
  def encode_strings(strs, max_len):
    ...
    for i, s in enumerate(strs):
      if hasattr(encoder, 'tokenize_string'):
        toks = encoder.tokenize_string(s)
      else:
